...
func resourceManager(takeLow chan Resource, takeHigh chan Resource, giveBack chan Resource) {
	//should be working, check with priorities in cfsg if unsure.
	...
	for {
		select {
		case takeHigh <- res:
			res = <-giveBack //giveback when we have a value to provide
		...
		default:
			...
			// from https://golangbyexample.com/select-default-case-go/:
			//default is executed if no send or receive operation in any of the case statements above is ready.
			//Placing lower priotity in the default state should therefore cause the lower priority to come later.
